...
import { effectiveStatus } from "@/lib/gig-machine";
import { fetchProfileNames } from "@/lib/sync";
import type { Gig, Message } from "@/lib/types";
...
export default function MessagesPage() {
  ...
  // One thread per counterparty — a company↔creator relationship can hold many
  // gigs. The row links to the most-recent gig (where the gig switcher lives).
  const byCounterparty = new Map<string, Gig[]>();
  for (const g of myGigs) {
    const cid = role === "creator" ? g.companyId : g.creatorId;
    const list = byCounterparty.get(cid) ?? [];
    list.push(g);
    byCounterparty.set(cid, list);
  }
  const threads = Array.from(byCounterparty.entries())
    .map(([counterpartyId, cpGigs]) => {
      const gigIds = new Set(cpGigs.map((g) => g.id));
      const last = [...messages].reverse().find((m) => gigIds.has(m.gigId));
      const latestGig = [...cpGigs].sort((a, b) => (b.createdAt ?? "").localeCompare(a.createdAt ?? ""))[0];
      const isUnread = !!(
        last &&
        last.senderId !== userId &&
        (!lastReadGigs?.[last.gigId] || last.createdAt > lastReadGigs[last.gigId])
      );
      return { counterpartyId, gigCount: cpGigs.length, last: last as Message | undefined, latestGig, isUnread };
    })
    .filter((t) => t.last)
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="mt-2 text-sm font-bold text-text-secondary">
          Every negotiation stays on-platform — offers and files in one thread.
        </p>
      </div>
      ...
      {threads.length === 0 ? (
        ...
        <div className="overflow-hidden rounded-[16px] border-2 border-ink bg-surface shadow-[3px_3px_0_0_#101805]">
          {threads.map(({ counterpartyId, gigCount, last, latestGig, isUnread }) => {
            const other =
              role === "creator"
                ? {
                    name: companyById(counterpartyId)?.name ?? profileNames[counterpartyId]?.name ?? "Brand",
                    hue: companyById(counterpartyId)?.logoHue ?? profileNames[counterpartyId]?.hue ?? 285,
                    src: undefined,
                  }
                : (() => {
                    const c = creators.find((c) => c.id === counterpartyId);
            ...
            return (
              <Link
                key={counterpartyId}
                href={`/gig/${latestGig.id}`}
                className={cn(
                  "flex items-center gap-3 border-b-2 border-ink/10 px-4 py-4 transition-colors last:border-0 hover:bg-surface-2/60",
              ...
              >
                <div className="relative shrink-0">
                  ...
                  {isUnread && (
                    <span className="absolute -right-0.5 -top-0.5 h-3 w-3 rounded-full bg-[#d6409f] ring-2 ring-surface" />
                  )}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-baseline justify-between gap-2">
                    <p className={cn("truncate text-sm", isUnread ? "font-extrabold" : "font-bold")}>{other.name}</p>
                    <span className="num shrink-0 text-[11px] text-text-tertiary">{daysAgo(last!.createdAt)}</span>
                  </div>
                  <p className={cn("truncate text-[13px]", isUnread ? "font-semibold text-text-primary" : "font-medium text-text-secondary")}>
                    {preview}
                  </p>
                  <p className="mt-0.5 truncate text-xs font-medium text-text-tertiary">
                    {latestGig.title}
                    {gigCount > 1 && <span className="text-text-tertiary/70"> · {gigCount} gigs</span>}
                  </p>
                </div>
                <StatusPill status={effectiveStatus(latestGig)} />
              </Link>
            );
          })}
        </div>
